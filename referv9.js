// register-v9-debug.js - Versão com debug para recursividade
const { 
    Connection, 
    Keypair, 
    PublicKey, 
  ...
  async function main() {
    ...
    try {
      ...
      if (isSlot3 && airdropInfo) {
        ...
        // Adicionar PDAs das semanas ANTES dos uplines
        ...
        // IMPORTANTE: Adicionar PDAs do airdrop dos uplines ANTES dos pares de uplines
        if (uplineAccounts.length > 0) {
          console.log("  ➕ Adicionando PDAs do airdrop dos uplines...");
          
          // Criar array temporário para as PDAs do airdrop
          ...
          // Adicionar primeiro as PDAs do airdrop
          mainRemainingAccounts = [...mainRemainingAccounts, ...uplineAirdropPDAs];
      ...
      if (isSlot3) {
        ...
        if (airdropInfo) {
          ...
          if (uplineAccounts.length > 0) {
            // Calcular onde começam os upline airdrop PDAs
            const uplineAirdropStart = 49;
            const uplineAirdropCount = uplineAccounts.length / 2;
            console.log(`  [${uplineAirdropStart}-${uplineAirdropStart + uplineAirdropCount - 1}]: Upline Airdrop PDAs (${uplineAirdropCount} contas)`);
            
            // Calcular onde começam os pares de uplines
            const uplinePairsStart = uplineAirdropStart + uplineAirdropCount;
            console.log(`  [${uplinePairsStart}-${uplinePairsStart + uplineAccounts.length - 1}]: Upline pairs (${uplineAccounts.length} contas = ${uplineAccounts.length/2} pares)`);
          }
        }
