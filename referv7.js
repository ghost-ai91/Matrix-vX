...
// Função para preparar PDAs de semanas pendentes
async function preparePendingWeeksPDAs(connection, airdropProgramId, currentWeek) {
  console.log("\n📅 PREPARANDO PDAs DE SEMANAS PENDENTES...");
  ...
  if (!stateAccountInfo) {
    console.log("  ⚠️ Não foi possível obter estado do programa");
    return [];
  }
  
  // Extrair timestamp (offset 97 para start_timestamp no layout do ProgramState)
  let startTimestamp;
  if (stateAccountInfo.data.length >= 105) {
    startTimestamp = Number(stateAccountInfo.data.readBigInt64LE(97));
  } else {
    console.log("  ⚠️ Formato de dados inesperado");
    return [];
  }
  
  const currentTimestamp = Math.floor(Date.now() / 1000);
  const elapsedSeconds = currentTimestamp - startTimestamp;
  const expectedWeek = Math.min(Math.floor(elapsedSeconds / WEEK_DURATION_SECONDS) + 1, 36);
  
  console.log(`  ⏱️ Timestamp de início: ${new Date(startTimestamp * 1000).toLocaleString()}`);
  console.log(`  ⏰ Tempo decorrido: ${elapsedSeconds}s`);
  console.log(`  📅 Semana atual do programa: ${currentWeek}`);
  console.log(`  📅 Semana esperada baseada no tempo: ${expectedWeek}`);
  
  const weekPDAs = [];
  
  // Incluir PDAs de semanas entre current e expected (exclusive)
  if (expectedWeek > currentWeek) {
    console.log(`  ⚠️ ${expectedWeek - currentWeek} semanas pendentes detectadas`);
    
    // IMPORTANTE: Não incluir currentWeek nem expectedWeek (já estão nas 7 contas base)
    for (let week = currentWeek + 1; week < expectedWeek; week++) {
      ...
      // Verificar se a PDA existe
      const weekInfo = await connection.getAccountInfo(weekPDA);
      const status = weekInfo ? "✅ Existe" : "⚠️ Será criada";
      
      console.log(`  📊 Semana ${week}: ${weekPDA.toString().substring(0, 8)}... ${status}`);
    }
    
    console.log(`  📦 Total de PDAs de semanas pendentes: ${weekPDAs.length}`);
  } else {
    console.log(`  ✅ Nenhuma semana pendente detectada`);
  }
...
async function main() {
  ...
  try {
    ...
    // SOLUÇÃO: Manter sempre 7 contas do airdrop na posição esperada
    if (isSlot3 && airdropInfo) {
      console.log("\n🔄 ADICIONANDO CONTAS DO AIRDROP AOS REMAINING ACCOUNTS");
      
      // IMPORTANTE: Adicionar EXATAMENTE 7 contas do airdrop na ordem esperada
      const airdropAccounts = [];
      
      // 1. Program State PDA
      airdropAccounts.push({
        pubkey: airdropInfo.programStatePda,
        isWritable: true,
        isSigner: false,
      });
      console.log(`  ➕ [6] Program State PDA: ${airdropInfo.programStatePda.toString()}`);
      ...
      console.log(`  ➕ [7] User Account PDA (AIRDROP): ${airdropInfo.userAccountPda.toString()}`);
      
      // 3. Current Week Data PDA
      airdropAccounts.push({
        pubkey: airdropInfo.currentWeekDataPda,
        isWritable: true,
        isSigner: false,
      });
      console.log(`  ➕ [8] Current Week Data PDA: ${airdropInfo.currentWeekDataPda.toString()}`);
      
      // 4. Next Week Data PDA
      airdropAccounts.push({
        pubkey: airdropInfo.nextWeekDataPda,
        isWritable: true,
        isSigner: false,
      });
      console.log(`  ➕ [9] Next Week Data PDA: ${airdropInfo.nextWeekDataPda.toString()}`);
      ...
      console.log(`  ➕ [10] Referrer Wallet: ${referrerAddress.toString()}`);
      
      // 6. Airdrop Program ID
      airdropAccounts.push({
        pubkey: VERIFIED_ADDRESSES.AIRDROP_PROGRAM_ID,
        isWritable: false,
        isSigner: false,
      });
      console.log(`  ➕ [11] Airdrop Program ID: ${VERIFIED_ADDRESSES.AIRDROP_PROGRAM_ID.toString()}`);
      ...
      console.log(`  ➕ [12] Instructions Sysvar: ${SYSVAR_INSTRUCTIONS_PUBKEY.toString()}`);
      
      // Verificar que temos exatamente 7 contas
      if (airdropAccounts.length !== 7) {
        throw new Error(`ERRO: Esperado 7 contas do airdrop, mas temos ${airdropAccounts.length}`);
      }
      
      // Adicionar as 7 contas do airdrop
      mainRemainingAccounts = [...mainRemainingAccounts, ...airdropAccounts];
      
      // Adicionar uplines APÓS as 7 contas do airdrop (começando no índice 13)
      mainRemainingAccounts = [...mainRemainingAccounts, ...uplineAccounts];
      
      // NOVO: Preparar PDAs de semanas pendentes
      const pendingWeeksPDAs = await preparePendingWeeksPDAs(
        connection,
        VERIFIED_ADDRESSES.AIRDROP_PROGRAM_ID,
        airdropInfo.currentWeek
      );
      
      // IMPORTANTE: Adicionar semanas pendentes APÓS os uplines
      // Isso não quebra a validação do contrato porque ele só verifica até o início dos uplines
      if (pendingWeeksPDAs.length > 0) {
        console.log(`\n📅 ADICIONANDO ${pendingWeeksPDAs.length} PDAs DE SEMANAS PENDENTES APÓS UPLINES`);
        mainRemainingAccounts = [...mainRemainingAccounts, ...pendingWeeksPDAs];
      }
      
      console.log(`\n📊 RESUMO FINAL DE CONTAS:`);
      console.log(`  Índices [0-3]: Vault A (4 contas)`);
      console.log(`  Índices [4-5]: Chainlink (2 contas)`);
      console.log(`  Índices [6-12]: Airdrop (7 contas) ✅`);
      console.log(`  Índices [13-${12 + uplineAccounts.length}]: Uplines (${uplineAccounts.length / 2} pares)`);
      if (pendingWeeksPDAs.length > 0) {
        const startIdx = 13 + uplineAccounts.length;
        console.log(`  Índices [${startIdx}-${startIdx + pendingWeeksPDAs.length - 1}]: Semanas pendentes (${pendingWeeksPDAs.length} PDAs)`);
      }
